...
class ExerciseStatusAdapter(val items :ArrayList<ExerciseModel>,val context: Context) :RecyclerView.Adapter<ExerciseStatusAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder:ViewHolder,position: Int){
        ...
        if(model.getisCompleted()==true){
            holder.tvItem.background=ContextCompat.getDrawable(context,R.drawable.compeleted_back)
            holder.tvItem.setTextColor(Color.parseColor("#636161"))
        }
        if(model.getisSelected()==true){
            holder.tvItem.background=ContextCompat.getDrawable(context,R.drawable.selected_back)
